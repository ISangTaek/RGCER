"""Single-GPU, one-shot P2 smoke. Formal training and test have no CLI here."""
import argparse
...
def main():
    ...
    release=load_release(a.authorization,a.authorization_sha,a.approval)
    ...
    source_initial=None;target_initial=None;sources={};completed=[];attempted_updates=0
    try:
        ...
        write_new(output/'environment.json',dict(commit=release['commit'],python=sys.version,torch=str(torch.__version__),
            ...
            deterministic=True,tf32=False,scope='SMOKE_ONLY'))
        for run in release['runs']:
            ...
            if is_source:
                ...
                if source_initial is None:source_initial=initial
                require(initial==source_initial,'source initial pairing')
            else:
                ...
                require(payload['identity']['run']['run_id']==run['source_run_id'] and payload['total_updates']==1,'smoke source identity')
                ...
                initial=state_digest(model.state_dict())
                if target_initial is None:target_initial=initial
                require(initial==target_initial,'target full initial pairing')
            ...
            def fresh():
                m=build_model('source',42) if is_source else build_model('target',42,encoder)
                return P2Engine(m,view,method=method,seed=42,run_identity=identity,device='cuda:0')
            engine=P2Engine(model,view,method=method,seed=42,run_identity=identity,device='cuda:0')
            ...
            while engine.total_updates<run['updates']:
                attempted_updates+=1
                require(attempted_updates<=112,'smoke update budget')
                record=engine.step()
                ...
                if method=='HF_low' and engine.total_updates==30:
            ...
            del engine
            if method!='HF_low':del model
            torch.cuda.empty_cache();torch.cuda.reset_peak_memory_stats(0)
        require(sum(r['updates'] for r in completed)==attempted_updates==112,'final smoke budget')
        write_new(output/'budget.json',dict(attempted_updates=attempted_updates,completed_updates=112,counts={r['run']['run_id']:r['updates'] for r in completed}))
        write_new(output/'summary.json',dict(execution_status='FINISHED',validation_status='PASS',acceptance_status='PENDING_REVIEW',
            delivery_kind='SMOKE_NOT_FORMAL',runs=completed,updates=112,test_access=False,calibration_access=False))
        ...
        print('SMOKE_COMPLETE_112_UPDATES_PENDING_CODEX_REVIEW',flush=True)
