...
# more than this cannot be Top-1.
...
def _load_reference(d6_root: Path, reference: str, seed: int, expected_last_epoch: int, stage_dir: str = "d6_stage_a") -> dict:
    ...
    return summarize_d6_reference(run_dir, reference, seed, expected_last_epoch)


def summarize_d6_reference(run_dir: Path | None, candidate: str, seed: int, expected_last_epoch: int) -> dict:
    """D6 references are re-summarised with the D6 contract (kept separate from
    the D7 contract so a mislabelled reference can never rank silently)."""

    from scripts.d6_aggregate import check_d6_run_contract

    row: dict = {"candidate": candidate, "seed": seed}
    if run_dir is None or not run_dir.exists():
        row["status"] = "MISSING_RUN"
        return row
    violations, _ = check_d6_run_contract(run_dir, candidate, seed, expected_last_epoch)
    if violations:
        row["status"] = "CONTRACT_VIOLATION"
        row["contract_violations"] = "; ".join(violations)
        return row
    ...
    row.update(
        {
            "status": "PASS",
            "stable_rmse": _mean(values[-STABLE_WINDOW:]),
...
def stage_a(d7_root: Path, d6_root: Path, seed: int, output_dir: Path, trace: dict) -> dict:
    ...
    drift_rows = _collect_drift_rows(d7_root, "d7_stage_a", seed)
    _write_csv(
        output_dir / "D7A_REPRESENTATION_DRIFT.csv",
        ["candidate", "seed", "epoch", "backbone_param_drift", "early_block_drift",
         "late_block_drift", "feature_drift", "human3_rmse"],
        drift_rows,
    )
...
def _collect_drift_rows(stage_root: Path, stage_dir: str, seed: int) -> list[dict]:
    rows = []
    for candidate in D7_CANDIDATES:
        run_dir = stage_root / stage_dir / candidate / f"d7_{candidate}_e20" / f"seed_{seed}"
        ...
        with drift_csv.open(newline="", encoding="utf-8") as handle:
            for entry in csv.DictReader(handle):
                ...
                if summary.is_file():
                    with summary.open(newline="", encoding="utf-8") as handle2:
                        for metric_row in csv.DictReader(handle2):
                            if int(metric_row["epoch"]) == epoch:
                                human_rmse = metric_row.get("val_human3_macro_rmse", "")
                                break
                rows.append(
                    {
                        "candidate": candidate,
                        "seed": seed,
                        "epoch": epoch,
                        "backbone_param_drift": entry["backbone_param_drift"],
                        "early_block_drift": entry["early_block_drift"],
                        "late_block_drift": entry["late_block_drift"],
...
def stage_b(d7_root: Path, d6_root: Path, candidates: list[str], seeds: list[int], output_dir: Path, trace: dict) -> dict:
    ...
    for candidate in candidates:
        for seed in seeds:
            run_dir = d7_root / "d7_stage_b" / candidate / f"d7_{candidate}_e40" / f"seed_{seed}"
            drift_csv = run_dir / "diagnostics" / "d7_representation_drift.csv"
            if not drift_csv.is_file():
                continue
            with drift_csv.open(newline="", encoding="utf-8") as handle:
    ...
    _write_csv(
        output_dir / "D7B_REPRESENTATION_DRIFT.csv",
        ["candidate", "seed", "epoch", "backbone_param_drift", "early_block_drift",
         "late_block_drift", "feature_drift"],
        drift_rows,
    )
    endpoint_rows = []
    for candidate in candidates:
...
def main() -> None:
    ...
    if args.mode == "stage-a":
        ...
        if not args.top2 or len(args.top2) != 2:
            raise SystemExit("stage-b requires --top2 with exactly two candidates")
        result = stage_b(
            Path(args.d7_root),
            Path(args.d6_root),
            list(args.top2),
