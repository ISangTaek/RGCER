...
def verify(root,release,release_sha,data,panel=None,route=None):
    import torch
    runs=selected_runs(release,panel,route);formal=release['stage']=='FORMAL'
    ...
    for run in runs:
        ...
        if formal or run['role']=='target':allowed.update(rid+f'/epoch_{e:02d}.json' for e in range(40 if formal else 6))
        if formal:allowed.add(rid+'/last.pt')
        if not formal and run.get('method')=='HF_low':allowed.add(rid+'/before_unfreeze.pt')
    ...
    require(summary['updates']==budget and summary['test_access'] is False and summary['calibration_access'] is False,'summary permissions/budget')
    require(len(summary['runs'])==len(runs),'summary run count')
    counts={};sources={};audits=[]
    for run,reported in zip(runs,summary['runs']):
        ...
        source_sha=None
        if is_source:
            view=data.source(run['panel'],run['route']);model=build_model('source',run['seed'])
        else:
            ...
            model=build_model('target',run['seed'],encoder)
        identity=dict(**run,release_sha=release_sha,commit=release['commit'],source_checkpoint_sha=source_sha)
        engine=P2Engine(model,view,method='SOURCE' if is_source else run['method'],seed=run['seed'],run_identity=identity)
        ...
        require(payload['epoch']==(40 if formal else (0 if is_source else 6)) and payload['offset']==(1 if is_source and not formal else 0),'checkpoint cursor')
        ...
        engine.control.restore(payload['optimization'],allow_partial_epoch=is_source and not formal)
        ...
        if is_source:
            ...
            audits.append(dict(run_id=rid,status='PASS',updates=run['updates']))
        else:
            ...
            audits.append(dict(run_id=rid,**check,updates=run['updates']))
        del engine,model
    require(read('budget.json')==dict(attempted_updates=budget,completed_updates=budget,counts=counts),'budget tape')
    return dict(content_status='PASS',acceptance_status='PENDING_CODEX_REVIEW',scope='read_only_no_forward',updates=budget,runs=audits)
...
def main(stage='SMOKE'):
    ...
    a=p.parse_args();release=load_release(a.authorization,a.authorization_sha,a.approval,stage)
    _,members=load_contract(a.protocol,a.members);data=P2Data(members,a.datastore)
    try:write_new(a.output,verify(a.results,release,a.authorization_sha,data,getattr(a,'panel',None),getattr(a,'route',None)))
    finally:data.close()
